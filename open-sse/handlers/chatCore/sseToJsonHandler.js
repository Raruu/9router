...
import { applyResponseModelOverride } from "../../utils/responseModel.js";
 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, responseModelOverride, customToolNames, toolNameMap, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, silent: true, ttft: totalLatency, totalLatency });
      ...
      // convertResponsesStreamToJson emits no `model` field, so a combo-name
      // override has nothing to rewrite here — left untouched by design rather
      // than inventing a field this shape never carried.
...
const finalRespOut = restoreToolNames(finalResp, toolNameMap);
      applyResponseModelOverride(finalRespOut, responseModelOverride);

      return { success: true, response: new Response(JSON.stringify(finalRespOut), { headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }) };
    } catch (err) {
...
const finalBodyOut = restoreToolNames(finalBody, toolNameMap);
    applyResponseModelOverride(finalBodyOut, responseModelOverride);

    return { success: true, response: new Response(JSON.stringify(finalBodyOut), { headers: { "Content-Type": "application/json", "Access-Control-Allow-Origin": "*" } }) };
  } catch (err) {
