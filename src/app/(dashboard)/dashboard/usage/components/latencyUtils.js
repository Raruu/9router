...
export function buildLatencyData(latencyByModel, metric = "total", hiddenKeys) {
  ...
  // Hidden rows drop out before the axis is derived, so hiding a slow outlier
  // rescales the domain instead of leaving it stretched. `all` keeps every
  // chart-eligible row so the picker enumerates from one source of truth.
  const all = eligible.map((e) => ({ key: e.key, label: e.label }));
  const rows = hiddenKeys?.size
    ? eligible.filter((e) => !hiddenKeys.has(e.key))
    : eligible;
  ...
  return { data, all, noisy, domain: [domainMin, domainMax], ticks, excluded: noisy.length };
}
